...
async authenticateUser(@Body() body, @Res() res: Response) {
  const { user_email, password, client_id, redirect_uri, scope, state } = body;

  // Authenticate the user with the provided credentials
  const isAuthenticated = await this.authService.validateUser(user_email, password);
  
  if (isAuthenticated) {
    // If authenticated, proceed to generate an authorization code
    const authorizationCode = await this.oauthService.generateAuthorizationCode(client_id, user_email, redirect_uri, scope);
console.log("something happened", user_email, password, client_id, redirect_uri, scope, state)
    ...
    res.render('auth-form', { error: 'Invalid credentials', client_id, redirect_uri, scope, state });
  }
}
